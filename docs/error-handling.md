...
| Mass deletion | A post-optimization, pre-execution cap blocks the entire plan when `delete_local` + `delete_remote` exceeds the configured maximum (default 20; `0` disables). Structural prevention still applies first. See [sync-pipeline.md § Deletion safety](sync-pipeline.md#deletion-safety). |
