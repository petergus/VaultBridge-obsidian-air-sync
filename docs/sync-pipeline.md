...
- Prunes no-ops via explicit cases (in order): both sides absent → keep only if a baseline exists (cleanup); no baseline → always keep (new file); exactly one side absent → always keep (a deletion or rename source on that side); otherwise keep iff `hasChanged(local)` or `hasRemoteChanged(remote)`. Entries with neither side nor a baseline are dropped first. The "remote absent, local present" case is load-bearing: the delta cursor advances past a remote deletion, so pruning it here would lose the deletion for good and the stale local copy would later resurrect on the remote.
- Also unions any `extraPaths` passed by the orchestrator (the paths of deletions currently held for review), so a held remote-origin deletion is re-decided every cycle even though the delta no longer reports it.
...
4. **Folder survivors** -- a directory `delete_local`/`delete_remote` is dropped from the plan (`protectFoldersWithSurvivors`) when any non-deletion action targets a path beneath it. Directory deletes are recursive and run after transfers/conflicts, so otherwise a note created inside a folder the other device deleted (or a modified note there that a conflict restores) would be trashed right after being kept.
5. **Held deletions** -- after rename optimization and before execution, `splitPlanAtLimit()` counts `delete_local` and `delete_remote` actions. If their combined count exceeds **Maximum deletions per sync** (default 20), or the rolling velocity cap (100 deletions/hour) would be exceeded, the deletions are held and everything else executes. While deletions are held: the checkpoint is not committed (a restart replays and re-holds them), their paths are re-evaluated every cycle via `extraPaths` (the in-memory cursor already moved past remote-origin ones), and the user is notified once per distinct held set. The **Approve held deletions** command does not replay the stale actions: it runs a fresh sync in which only deletions still planned for an approved (action, path) pair bypass the guards. Rename and state-only cleanup actions do not count.
...
- **Folder renames** (`coalesceLocalFolderRenames`): When a folder rename is detected, coalesces all descendant file rename actions into a single `rename_remote` with `isFolder: true`. The folder rename event is authoritative, so a descendant whose content also changed (Obsidian rewrote its links) — or can't be hash-verified — still moves with the folder and is listed in `changedDescendants`; the executor uploads it right after the move (see `rename-content.ts`). Coalescing is skipped only for a descendant that isn't a clean `delete_remote`+`push` pair or a new file pushed under the new prefix; uncoalesced file renames then fall through to individual file rename optimization.
- **Folder endpoints**: folders carry sync records, so a folder move also plans the folder's own `push(new)` (mkdir) and `delete_remote(old)` (recursive delete). Both coalescers consume these with the move. Left in the plan, the Phase 1 mkdir occupies the destination so the Phase 3 rename fails "Destination already exists", and the recursive delete then trashes every file that should have moved — which the other device receives as a mass deletion.
...
### Content-match fallback (`optimize-heuristic-renames.ts`)

A move can arrive without rename information: a local move made outside Obsidian (or before the tracker was initialized), a cold full-scan reconcile (which has no delta, so no remote rename pairs), or a backend delta that surfaced a move as delete + add. After the authoritative optimizers, each side falls back to a content match: an unmatched `delete_remote(old)` + no-baseline `push(new)` (local; SHA-256 + size) or `delete_local(old)` + no-baseline `pull(new)` (remote; same-algorithm `remoteChecksum` or hash + size against the old baseline) becomes a rename. Empty files never match, and among several identical candidates only a same-file-name one is taken — otherwise the pair stays a delete + transfer.
...
`executePlan()` in `plan-executor.ts` classifies each action by **resource lane** (which filesystem it mutates: `remote` / `local` / `both` / state-only) and **dependency tier** (`transfer` / `rename` / `delete` / state-only), then runs four phases separated by barriers:
...
| 4 — Prune | (derived from succeeded deletes/renames) | `pruneEmptyParentFolders()` (`prune-empty-folders.ts`) walks up from each affected parent, deepest first, deleting a directory only once it lists empty | A delete/rename can leave a parent directory empty; best-effort cleanup (a listing/delete error stops that branch, never fails the cycle). Runs last, after the structural mutations it cleans up after |

The phases run behind **sequential barriers** (Phase 1 fully drains before Phase 2 before Phase 3 before Phase 4 — the prune runs last, on the structural mutations' results). This preserves two safety properties: no content write (Phase 1) runs concurrently with a same-subtree structural rename/delete (Phase 3), and conflict (Phase 2, which touches both sides + a sibling path) never overlaps either. Renames stay serial so the rename optimizer's destination-occupancy assumptions hold; pooled deletes are safe even for the legitimate folder+descendant overlap via the inline delete CAS guard (the folder's `removeTree` evicts the child entry, so the child delete short-circuits) — see [ADR 0001 → T7](adr/0001-metadata-cache-is-subordinate-to-commit-last.md).
...
|---------|-------|-----------|
| Vault change | `create` / `modify` / `delete` | Marks path dirty via `localTracker.markDirty()`, then schedules a debounced sync (default 5 s; user-configurable via `syncDebounceSec`, 1 s floor). Consecutive edits reset the timer so sync fires the debounce window after the last change. |
| Vault rename | `rename` | Calls `localTracker.markRenamed(newPath, oldPath)` which records the rename pair and marks both paths dirty, then schedules a debounced sync. Folder targets call `markFolderRenamed` instead. If either endpoint is ignore-excluded, the rename is not recorded as a pair; each non-excluded endpoint is marked dirty, and the debounce fires only if at least one endpoint is non-excluded. |
...
The edit-debounce window is **configurable** (`syncDebounceSec`, default 5 s, 1 s floor). Because Obsidian's `debounce()` bakes its timeout in at creation, the scheduler routes edits through `scheduleDebouncedSync()`, which recreates the debouncer when the live value changes — so a longer "battery saver" window applies without re-wiring.

The **automatic** triggers are additionally **offline-gated** (`pauseSyncWhenOffline`, default on): while `navigator.onLine` is false, the debounced-sync callback, the foreground-resume signals, and the file-open priority pull early-return, so the device doesn't wake the radio for doomed network calls + retry backoff. Edits keep accumulating in the dirty set (it is cleared only by a successful cycle's `acknowledge()`) and flush on the next `online` event — which is **never** gated, because the device is online there by definition. Manual "Sync now" (command / status-bar click) is not gated, so the user can always force an attempt.
