...
export interface IBackendProvider {
	 ...
	 */
	getRemoteVaultDisplayPath?(settings: VaultBridgeSettings, logger?: Logger): Promise<string | null>;
	 ...
	 */
	clearCheckpointStore?(settings: VaultBridgeSettings): Promise<void>;

	/**
	 * Disconnect the backend: revoke auth and reset all backend state.
	 * Returns the reset backendData to persist.
	 */
	disconnect(settings: VaultBridgeSettings): Promise<Record<string, unknown>>;
...
export interface WebFolderPicker {
	 ...
	 */
	startWebFolderPick(settings: VaultBridgeSettings): Promise<Record<string, unknown>>;
...
export function getBackendData<T>(settings: VaultBridgeSettings): T | undefined {
